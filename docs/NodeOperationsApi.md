...
> NodePowerOperationQueued create_node_power_operation(fleet_id, node_id, node_power_operation, idempotency_key=idempotency_key)
...
with vpcloud_client.ApiClient(configuration) as api_client:
    ...
    try:
        # Queue a node power operation
        api_response = api_instance.create_node_power_operation(fleet_id, node_id, node_power_operation, idempotency_key=idempotency_key)
