...
class UpdateStorageViewRequest(BaseModel):
    """
    Request to update the size/quotas of an existing physical VAST storage view. The mount point is taken from the URL path.
    """ # noqa: E501
    size: Optional[StrictInt] = Field(default=None, description="New provisioned size in bytes.")
    soft_limit: Optional[StrictInt] = Field(default=None, description="New soft quota in bytes.", alias="softLimit")
    hard_limit: Optional[StrictInt] = Field(default=None, description="New hard quota in bytes.", alias="hardLimit")
    __properties: ClassVar[List[str]] = ["size", "softLimit", "hardLimit"]
